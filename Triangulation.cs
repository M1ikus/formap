...
public static class Triangulation
{
    ...
    public static List<int> Triangulate(List<Vector2> vertices)
    {
        ...
        // ═══════════════════════════════════════════════════════════════════
        // FIX: For very large polygons, simplify MORE aggressively instead of 
        ...
        while (cleaned.Count > 3 && guard < maxIterations)
        {
            ...
            if (!earClipped)
            {
                ...
                if (!removed)
                {
                    ...
                    {
                        // ═══════════════════════════════════════════════════════════
                        // FIX: Use convex decomposition instead of fan triangulation!
    ...
    /// <summary>
    /// Triangulates a polygon with holes and returns both the merged vertices and indices.
    /// This is needed because merging holes changes the vertex list.
    /// </summary>
    public static (List<Vector2> mergedVertices, List<int> indices) TriangulateWithHolesAndGetVertices(
        List<Vector2> vertices, List<int> holeStarts)
    {
        if (holeStarts.Count == 0)
        {
            return (new List<Vector2>(vertices), Triangulate(vertices));
        }

        // Merge holes with outer polygon first
        var merged = MergeHolesWithOuter(vertices, holeStarts);
        if (merged == null || merged.Count < 3)
            return (new List<Vector2>(), new List<int>());

        // Triangulate the merged polygon
        var indices = Triangulate(merged);
        return (merged, indices);
    }

    /// <summary>
    /// Merges holes into the outer polygon by connecting each hole to the outer boundary.
    /// Uses the standard algorithm: sort holes by rightmost X, then merge from right to left.
    /// </summary>
    public static List<Vector2>? MergeHolesWithOuter(List<Vector2> vertices, List<int> holeStarts)
    {
        if (vertices.Count == 0 || holeStarts.Count == 0)
            return new List<Vector2>(vertices);

        // Sort holes by start index (ascending) to extract them correctly
        var sortedHoleIndices = holeStarts.OrderBy(h => h).ToList();

        // Extract outer polygon (from 0 to first hole)
        var outer = new List<Vector2>();
        int outerEnd = sortedHoleIndices[0];
        for (int i = 0; i < outerEnd; i++)
        {
            outer.Add(vertices[i]);
        }

        // Remove duplicate closing vertex if present
        if (outer.Count > 2 && NearlyEqual(outer[0], outer[outer.Count - 1]))
        {
        ...
        for (int hIdx = 0; hIdx < sortedHoleIndices.Count; hIdx++)
        {
            int holeStart = sortedHoleIndices[hIdx];
            int holeEnd = (hIdx < sortedHoleIndices.Count - 1) ? sortedHoleIndices[hIdx + 1] : vertices.Count;
            ...
            if (hole.Count > 2 && NearlyEqual(hole[0], hole[hole.Count - 1]))
            {
        ...
        // CRITICAL: Sort holes by their rightmost X coordinate (descending)
        // This ensures we process rightmost holes first, so bridge insertions don't mess up indices
        holes = holes.OrderByDescending(h => h.Max(v => v.X)).ToList();

        // Merge each hole into outer
        ...
        for (int holeIndex = 0; holeIndex < holes.Count; holeIndex++)
        {
            ...
            // Get remaining holes (not yet merged) to check bridge intersection
            var remainingHoles = holes.Skip(holeIndex + 1).ToList();

            // Find bridge point on current result polygon
            int bridgeIdx = FindBridgePointWithOtherHoles(result, hole, remainingHoles);
            if (bridgeIdx < 0)
            {
                // Can't find valid bridge - skip this hole
                continue;
            }

            // Find rightmost vertex of hole
            int holeRightmostIdx = FindRightmostVertex(hole);

            // Build hole sequence starting from rightmost vertex, then back to it
            var holeSequence = new List<Vector2>();
            for (int i = 0; i < hole.Count; i++)
            {
                int idx = (holeRightmostIdx + i) % hole.Count;
                holeSequence.Add(hole[idx]);
            }
            // Close back to rightmost
            holeSequence.Add(hole[holeRightmostIdx]);

            // Insert: bridge point -> hole sequence -> back to bridge point
            // This creates the bridge: outer[bridgeIdx] -> hole[rightmost] -> ... -> hole[rightmost] -> outer[bridgeIdx]
            var bridgePoint = result[bridgeIdx];
            result.Insert(bridgeIdx + 1, bridgePoint); // duplicate bridge point at end
            result.InsertRange(bridgeIdx + 1, holeSequence); // insert hole between
        }

        // Final cleanup - remove consecutive duplicates
        result = CleanDuplicateVertices(result);

        if (result.Count < 3)
            return null;

        // Ensure final result is CCW
        if (!IsCCW(result))
        {
            result.Reverse();
        }
    ...
    private static List<Vector2> CleanDuplicateVertices(List<Vector2> vertices)
    {
        var cleaned = new List<Vector2>();
        for (int i = 0; i < vertices.Count; i++)
        {
            if (i == 0 || !NearlyEqual(vertices[i], vertices[i - 1]))
                cleaned.Add(vertices[i]);
        }
        return cleaned;
    }

    private static int FindRightmostVertex(List<Vector2> polygon)
    {
        ...
        {
            if (polygon[i].X > maxX || (polygon[i].X == maxX && polygon[i].Y > polygon[rightmost].Y))
            {
                maxX = polygon[i].X;
                rightmost = i;
            }
    ...
    /// <summary>
    /// Finds a bridge point that doesn't intersect outer, current hole, or any remaining holes
    /// </summary>
    public static int FindBridgePointWithOtherHoles(List<Vector2> outer, List<Vector2> hole, List<List<Vector2>> otherHoles)
    {
        if (outer.Count == 0 || hole.Count == 0)
            return -1;

        int holeRightmostIdx = FindRightmostVertex(hole);
        Vector2 holePoint = hole[holeRightmostIdx];
        ...
        for (int i = 0; i < outer.Count; i++)
        {
            ...
            // Check if bridge doesn't intersect outer polygon edges
            bool isValidBridge = true;
            for (int j = 0; j < outer.Count; j++)
            {
                int next = (j + 1) % outer.Count;

                // Skip edges adjacent to candidate point
                if (j == i || next == i || j == ((i - 1 + outer.Count) % outer.Count))
                    continue;

                if (SegmentIntersectProper(p, holePoint, outer[j], outer[next]))
                {
                    isValidBridge = false;
                    break;
                }
            ...
            // Check if bridge doesn't intersect current hole edges
            if (isValidBridge)
            {
                for (int j = 0; j < hole.Count; j++)
                {
                    int next = (j + 1) % hole.Count;

                    // Skip edges adjacent to hole's rightmost point
                    if (j == holeRightmostIdx || next == holeRightmostIdx)
                        continue;

                    if (SegmentIntersectProper(p, holePoint, hole[j], hole[next]))
                    {
                        isValidBridge = false;
                        break;
                    }
            ...
            // Check if bridge doesn't intersect any OTHER hole edges
            if (isValidBridge)
            {
                foreach (var otherHole in otherHoles)
                {
                    for (int j = 0; j < otherHole.Count; j++)
                    {
                        int next = (j + 1) % otherHole.Count;

                        if (SegmentIntersectProper(p, holePoint, otherHole[j], otherHole[next]))
                        {
                            isValidBridge = false;
                            break;
                        }
                    }
                    if (!isValidBridge) break;
                }
            }

            if (isValidBridge)
            {
                float dist = Vector2.DistanceSquared(p, holePoint);
                if (dist < minDist)
                {
                    minDist = dist;
                    bestIdx = i;
        ...
        // Fallback - just find closest point (may cause issues but better than nothing)
        if (bestIdx < 0)
        {
            for (int i = 0; i < outer.Count; i++)
            {
                float dist = Vector2.DistanceSquared(outer[i], holePoint);
                if (dist < minDist)
                {
                    minDist = dist;
                    bestIdx = i;
    ...
    {
        // Create index list [0, 1, 2, ..., n-1] and use existing PolygonArea method
        var indices = new List<int>(vertices.Count);
        for (int i = 0; i < vertices.Count; i++)
            indices.Add(i);
        return PolygonArea(indices, vertices) > 0f;
    }
}